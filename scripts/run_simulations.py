...
def simulator(budget_euros, strategy='yield', min_size_m2=65):
    ...
    if strategy == 'yield':
        ...
        # Buscamos alto crecimiento histórico + estar infravalorado
    ...
    # Aplicamos el mapeo de nombres de segmento
...
if res1 is not None:
    print(res1[['barrio_nombre', 'avg_venta_23', 'gross_yield', 'segmento_nombre']].to_string(index=False))

print("\n--- 💎 E2: BUSCADOR DE VALOR/GANGAS (Presupuesto: 350,000€) ---")
res2 = simulator(350000, strategy='safe')
if res2 is not None:
    print(res2[['barrio_nombre', 'avg_venta_23', 'desviacion_valor', 'segmento_nombre']].to_string(index=False))
...
# Filtramos directamente el segmento Prime (0)
prime_reco = df_ml[df_ml['segmento'] == 0].sort_values('avg_venta_23', ascending=False).head(3)
prime_reco['segmento_nombre'] = prime_reco['segmento'].map(segment_map)
print(prime_reco[['barrio_nombre', 'avg_venta_23', 'renta_bruta_llar', 'segmento_nombre']].to_string(index=False))
